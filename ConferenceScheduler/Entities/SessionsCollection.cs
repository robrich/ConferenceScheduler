...
namespace ConferenceScheduler.Entities
{
    ...
    public class SessionsCollection : List<Session>
    {
        ...
        /// <returns>The session object that was added to the collection</returns>
        public Session Add(int id, params Presenter[] presenters)
        {
            var session = CreateSession(id, presenters);
            this.Add(session);
            return session;
        }

        private static Session CreateSession(int id, IEnumerable<Presenter> presenters)
        {
            return new Session()
            {
                Id = id,
        ...
        private IEnumerable<Presenter> GetPresenters(int id, IEnumerable<Presenter> presenters)
        {
